...
🗺️ 简单估算一下一个数字型的数组，也就是`number[]`类型数据，占据多少内存。
...
不收集所有组合，每一次生成组合就去执行后续逻辑，而不是把所有组合都放在一个大数组里(`[组合1，组合2，...所有组合]`),再通过遍历的方式去执行后续逻辑。⚠️ 但是这样有有个问题，就是这么多个组合(5497 亿)，这种计算量级，不是一个普通服务器能跑完的，在这个项目中，我用 mac pro(8 核 16G)启动，跑 326 万个组合要 `10分钟`以上。所以也就只能优化算法，没错后面一篇是讲述我优化算法的经历。
